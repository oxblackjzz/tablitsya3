...
window.printLabel = function (htmlContent, copies) {
    // Створюємо приховане iframe для друку
    ...
    if (copies > 1) {
        ...
        if (bodyMatch) {
            ...
            for (let i = 0; i < copies; i++) {
                multipleLabels += `<div style="page-break-after: always;">${bodyContent}</div>`;
            }
            fullContent = htmlContent.replace(/<body[^>]*>[\s\S]*<\/body>/i, `<body>${multipleLabels}</body>`);
        }
    }
...
// Друк бірки в новому вікні (альтернативний метод)
window.printLabelInNewWindow = function (htmlContent, copies) {
    const printWindow = window.open('', '_blank', 'width=450,height=350');

    if (!printWindow) {
        alert('Будь ласка, дозвольте спливаючі вікна для друку бірок');
        return;
    }
