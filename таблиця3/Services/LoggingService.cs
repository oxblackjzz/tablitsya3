...
using AppLogLevel = таблиц€3.Models.LogLevel;
...
namespace таблиц€3.Services
{
    public class LoggingService
    {
        ...
        {
            _logs.Clear();
            LogInfo("Ћоги були очищен≥", "LoggingService");
            OnLogAdded?.Invoke();
        }
        ...
        public string ExportLogsAsText()
        {
            ...
            sb.AppendLine("=== ≈кспорт лог≥в ===");
            sb.AppendLine($"«генеровано: {DateTime.Now:dd.MM.yyyy HH:mm:ss}");
            sb.AppendLine($"¬сього запис≥в: {_logs.Count}");
            sb.AppendLine();
            ...
            foreach (var log in _logs.OrderByDescending(l => l.Timestamp))
            {
                ...
                {
                    sb.AppendLine($"  ƒетал≥: {log.Details}");
                }
